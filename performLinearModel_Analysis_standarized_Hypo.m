...
if (run_analysis)
    ...
    cn_full_lm_residuals = zeros(1,2000);
    cn_full_lm_rsqred = zeros(1,2000);

    for k = 1:2000
        cn_full_XMat = [cn_full_av45_amylodosis(:,k) cn_full_fdg_bl_avgMat(:,k) cn_full_av45_bl_avgMat(:,k) cn_age_data cn_gender_data cn_apoe4_data];
        cn_full_lm = LinearModel.fit(zscore(cn_full_XMat), zscore(cn_full_fdg_hypomet(:,k)), 'CategoricalVars', [5,6]);
        ...
        [h, p, s] = swtest(cn_full_lm.Residuals.Raw(:));
        cn_full_lm_residuals(k) = p;
    ...
    emci_full_lm_residuals = zeros(1,2000);
    emci_full_lm_rsqred = zeros(1,2000);

    for k = 1:2000
        emci_full_XMat = [emci_full_av45_amylodosis(:,k) emci_full_fdg_bl_avgMat(:,k) emci_full_av45_bl_avgMat(:,k) emci_age_data emci_gender_data emci_apoe4_data];
        emci_full_lm = LinearModel.fit(zscore(emci_full_XMat), zscore(emci_full_fdg_hypomet(:,k)), 'CategoricalVars', [5,6]);
        ...
        [h, p, s] = swtest(emci_full_lm.Residuals.Raw(:));
        emci_full_lm_residuals(k) = p;
    ...
    lmci_full_lm_residuals = zeros(1,2000);
    lmci_full_lm_rsqred = zeros(1,2000);

    for k = 1:2000
        lmci_full_XMat = [lmci_full_av45_amylodosis(:,k) lmci_full_fdg_bl_avgMat(:,k) lmci_full_av45_bl_avgMat(:,k) lmci_age_data lmci_gender_data lmci_apoe4_data];
        lmci_full_lm = LinearModel.fit(zscore(lmci_full_XMat), zscore(lmci_full_fdg_hypomet(:,k)), 'CategoricalVars', [5,6]);
        ...
        lmci_full_lm_residuals(k) = swtest(lmci_full_lm.Residuals.Raw(:));
    ...
    ad_full_lm_residuals = zeros(1,2000);
    ad_full_lm_rsqred = zeros(1,2000);

    for k = 1:2000
        ad_full_XMat = [ad_full_av45_amylodosis(:,k) ad_full_fdg_bl_avgMat(:,k) ad_full_av45_bl_avgMat(:,k) ad_age_data ad_gender_data ad_apoe4_data];
        ad_full_lm = LinearModel.fit(zscore(ad_full_XMat), zscore(ad_full_fdg_hypomet(:,k)), 'CategoricalVars', [5,6]);
        ...
        ad_full_lm_residuals(k) = swtest(ad_full_lm.Residuals.Raw(:));
...
if (run_writeStats)
    for i = {'cn', 'emci', 'lmci', 'ad'}
       for j = {'amyloidosis' , 'blFDG' , 'blAV45' , 'age' , 'gender' , 'apoe4'}
          s_e = strcat('writeToFile(',i,'_full_lm_',j,'_e_list_fdr, ''/home/sulantha/Desktop/linear_model_sept2014/Wo_Int_Std/',i,'_lm_',j,'_fdr_e_l.txt'', ''/home/sulantha/Desktop/linear_model_sept2014/Wo_Int_Std/',i,'_lm_',j,'_fdr_e_r.txt'', l_map_dToS, r_map_dToS, l_map_SpacialToData, r_map_SpacialToData);');
          eval(s_e{1,1});
       end
    end
    ...
    for i = {'cn', 'emci', 'lmci', 'ad'}
       for j = {'amyloidosis' , 'blFDG' , 'blAV45' , 'age' , 'gender' , 'apoe4'}
          s_t = strcat('writeToFile(',i,'_full_lm_',j,'_t_list_fdr, ''/home/sulantha/Desktop/linear_model_sept2014/Wo_Int_Std/',i,'_lm_',j,'_fdr_t_l.txt'', ''/home/sulantha/Desktop/linear_model_sept2014/Wo_Int_Std/',i,'_lm_',j,'_fdr_t_r.txt'', l_map_dToS, r_map_dToS, l_map_SpacialToData, r_map_SpacialToData);');
          eval(s_t{1,1});
       end
    end
    ...
    for i = {'cn', 'emci', 'lmci', 'ad'}
       for j = {'amyloidosis' , 'blFDG' , 'blAV45' , 'age' , 'gender' , 'apoe4'}
          s_t_f = strcat('writeToFile(',i,'_full_lm_',j,'_t_list, ''/home/sulantha/Desktop/linear_model_sept2014/Wo_Int/',i,'_lm_',j,'_full_t_l.txt'', ''/home/sulantha/Desktop/linear_model_sept2014/Wo_Int/',i,'_lm_',j,'_full_t_r.txt'', l_map_dToS, r_map_dToS, l_map_SpacialToData, r_map_SpacialToData);');
          eval(s_t_f{1,1});
       end
    end
end