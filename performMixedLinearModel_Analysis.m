...
if (run_analysis) 
    main_table = table(nominal(id_vector), nominal(visit_vector), age_at_visit_vector, nominal(gender_vector), nominal(apoe4_vector), nominal(diagnosis_vector), zeros(total_subjects*2, 1), zeros(total_subjects*2, 1), 'VariableNames', {'id','visit','age','gender','apoe4','diagnosis', 'fdg','av45'});
    pVals_emci = zeros(2000,1);
    eVals_emci = zeros(2000,1);
    tVals_emci = zeros(2000,1);
    
    pVals_lmci = zeros(2000,1);
    eVals_lmci = zeros(2000,1);
    tVals_lmci = zeros(2000,1);
    
    pVals_ad = zeros(2000,1);
    eVals_ad = zeros(2000,1);
    tVals_ad = zeros(2000,1);
    parfor k=1:2000
        for_table = main_table;
        for_table.fdg = fdg_matrix(:,k);
        for_table.av45 = av45_matrix(:,k);
        
        lme = fitlme(for_table, 'fdg ~ diagnosis*av45 + gender + apoe4 + (age|id)');
        
        pVals_emci(k) = lme.Coefficients(8,6).pValue;
        pVals_lmci(k) = lme.Coefficients(9,6).pValue;
        pVals_ad(k) = lme.Coefficients(10,6).pValue;
        
        eVals_emci(k) = lme.Coefficients(8,2).Estimate;
        eVals_lmci(k) = lme.Coefficients(9,2).Estimate;
        eVals_ad(k) = lme.Coefficients(10,2).Estimate;
        
        tVals_emci(k) = lme.Coefficients(8,4).tStat;
        tVals_lmci(k) = lme.Coefficients(8,4).tStat;
        tVals_ad(k) = lme.Coefficients(8,4).tStat;
...
if (run_fdr)
    pVals_emci_fdr = fdr_bh(pVals_emci, 0.1);
    pVals_lmci_fdr = fdr_bh(pVals_lmci, 0.1);
    pVals_ad_fdr = fdr_bh(pVals_ad, 0.1);
end


if (get_estimates)
    eVals_emci_fdr = eVals_emci .* pVals_emci_fdr;
    eVals_lmci_fdr = eVals_lmci .* pVals_lmci_fdr;
    eVals_ad_fdr = eVals_ad .* pVals_ad_fdr;
end

if (get_tstats_fdr)
    tVals_emci_fdr = tVals_emci .* pVals_emci_fdr;
    tVals_lmci_fdr = tVals_lmci .* pVals_lmci_fdr;
    tVals_ad_fdr = tVals_ad .* pVals_ad_fdr;
end
...
if (run_writeStats)
    for i = {'emci', 'lmci', 'ad'}
          ...
          s_e = strcat('writeToFile(eVals_',i,'_fdr'', ''/home/sulantha/Desktop/linear_model_sept2014/mixedModel/',i,'_lm_fdr_e_l.txt'', ''/home/sulantha/Desktop/linear_model_sept2014/mixedModel/',i,'_lm_fdr_e_r.txt'', l_map_dToS, r_map_dToS, l_map_SpacialToData, r_map_SpacialToData);');
          eval(s_e{1,1});
    ...
    for i = {'emci', 'lmci', 'ad'}
          ...
          s_t = strcat('writeToFile(tVals_',i,'_fdr'', ''/home/sulantha/Desktop/linear_model_sept2014/mixedModel/',i,'_lm_fdr_t_l.txt'', ''/home/sulantha/Desktop/linear_model_sept2014/mixedModel/',i,'_lm_fdr_t_r.txt'', l_map_dToS, r_map_dToS, l_map_SpacialToData, r_map_SpacialToData);');
          eval(s_t{1,1});
    ...
    for i = {'emci', 'lmci', 'ad'}
          ...
          s_t_f = strcat('writeToFile(tVals_',i,''', ''/home/sulantha/Desktop/linear_model_sept2014/mixedModel/',i,'_lm_full_t_l.txt'', ''/home/sulantha/Desktop/linear_model_sept2014/mixedModel/',i,'_lm_full_t_r.txt'', l_map_dToS, r_map_dToS, l_map_SpacialToData, r_map_SpacialToData);');
          eval(s_t_f{1,1});
