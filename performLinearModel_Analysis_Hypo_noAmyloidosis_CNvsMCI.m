...
if (gen_data_tables)
    ...
    total_subjects = cn_size + emci_size + lmci_size;
    
    age_vec = [cn_age_data; emci_age_data; lmci_age_data];
    gender_vec = [cn_gender_data; emci_gender_data; lmci_gender_data];
    apoe4_vec = [cn_apoe4_data; emci_apoe4_data;lmci_apoe4_data];
    diagnosis_vector = [ones(cn_size,1)*0; ones(emci_size,1)*1; ones(lmci_size,1)*1];
    
    fdg_bl_matrix = [cn_full_fdg_bl_avgMat; emci_full_fdg_bl_avgMat; lmci_full_fdg_bl_avgMat];
    fdg_fu_matrix = [cn_full_fdg_fu_avgMat; emci_full_fdg_fu_avgMat; lmci_full_fdg_fu_avgMat];
    av45_bl_matrix = [cn_full_av45_bl_avgMat; emci_full_av45_bl_avgMat; lmci_full_av45_bl_avgMat];
    fdg_hypo_matrix = [cn_full_fdg_Yearlyhypomet; emci_full_fdg_Yearlyhypomet; lmci_full_fdg_Yearlyhypomet];
    av45_amy_matrix = [cn_full_av45_Yearlyamyloidosis; emci_full_av45_Yearlyamyloidosis; lmci_full_av45_Yearlyamyloidosis];
...
if (run_analysis) 
    ...
    pVals_mciblAV45 = zeros(2000,1);
        
    eVals_blAV45 = zeros(2000,1);
    eVals_mciblAV45 = zeros(2000,1);
        
    tVals_blAV45 = zeros(2000,1);
    tVals_mciblAV45 = zeros(2000,1);
    ...
    for k=1:2000
        ...
        pVals_mciblAV45(k) = lm.Coefficients.pValue(8);
        
        eVals_blAV45(k) = lm.Coefficients.Estimate(7);
        eVals_mciblAV45(k) = lm.Coefficients.Estimate(8);
          
        tVals_blAV45(k) = lm.Coefficients.tStat(7);
        tVals_mciblAV45(k) = lm.Coefficients.tStat(8);
...
if (run_fdr)
    ...
    pVals_mciblAV45_fdr = fdr_bh(pVals_mciblAV45, 0.1);
...
if (get_estimates)
    ...
    eVals_mciblAV45_fdr = eVals_mciblAV45 .* pVals_mciblAV45_fdr;
end

if (get_tstats_fdr)
    tVals_blAV45_fdr = tVals_blAV45 .* pVals_blAV45_fdr;
    tVals_mciblAV45_fdr = tVals_mciblAV45 .* pVals_mciblAV45_fdr;
end

if (run_writeStats)
    for i = {'blAV45', 'mciblAV45'}
       
          s_e = strcat('writeToFile(eVals_',i,'_fdr'', ''/data/data03/sulantha/linear_model_feb2015/Int_diagnosis_Hypo_diffBlur_noAmyloidosis_CNvsMCI/',i,'_lm_fdr_e_l.txt'', ''/data/data03/sulantha/linear_model_feb2015/Int_diagnosis_Hypo_diffBlur_noAmyloidosis_CNvsMCI/',i,'_lm_fdr_e_r.txt'', l_map_dToS, r_map_dToS, l_map_SpacialToData, r_map_SpacialToData);');
          eval(s_e{1,1});
    ...
    for i = {'blAV45', 'mciblAV45'}
          ...
          s_t = strcat('writeToFile(tVals_',i,'_fdr'', ''/data/data03/sulantha/linear_model_feb2015/Int_diagnosis_Hypo_diffBlur_noAmyloidosis_CNvsMCI/',i,'_lm_fdr_t_l.txt'', ''/data/data03/sulantha/linear_model_feb2015/Int_diagnosis_Hypo_diffBlur_noAmyloidosis_CNvsMCI/',i,'_lm_fdr_t_r.txt'', l_map_dToS, r_map_dToS, l_map_SpacialToData, r_map_SpacialToData);');
          eval(s_t{1,1});
    ...
    for i = {'blAV45', 'mciblAV45'}
          ...
          s_t_f = strcat('writeToFile(tVals_',i,''', ''/data/data03/sulantha/linear_model_feb2015/Int_diagnosis_Hypo_diffBlur_noAmyloidosis_CNvsMCI/',i,'_lm_full_t_l.txt'', ''/data/data03/sulantha/linear_model_feb2015/Int_diagnosis_Hypo_diffBlur_noAmyloidosis_CNvsMCI/',i,'_lm_full_t_r.txt'', l_map_dToS, r_map_dToS, l_map_SpacialToData, r_map_SpacialToData);');
          eval(s_t_f{1,1});
